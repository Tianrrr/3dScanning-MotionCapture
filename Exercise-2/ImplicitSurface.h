...
public:
	...
	double Eval(const Eigen::Vector3d& _x)
	{
		...
		double result = 0.0;
		for (uint i=0; i<m_numCenters+4;i++)
		{
			if(i<m_numCenters)
			{
			...
			{
				result += _x[0] * m_coefficents [i];
				result += _x[1] * m_coefficents [i+1];
				result += _x[2] * m_coefficents [i+2];
...
#define phi(i,j) EvalBasis((m_funcSamp.m_pos[i]-m_funcSamp.m_pos[j]).norm())
	...
	void BuildSystem()
	{
		...
		for (uint row=0; row < 2*m_numCenters; row++)
			for (uint col=0; col < m_numCenters+4; col++)
			{
				if (row<m_numCenters && col<m_numCenters)
				{
					A(row, col) = phi(row, col);
				}
				else if(row<m_numCenters && col==m_numCenters)
				{
					A(row,m_numCenters) = m_funcSamp.m_pos[row][0];
					A(row,m_numCenters+1) = m_funcSamp.m_pos[row][1];
					A(row,m_numCenters+2) = m_funcSamp.m_pos[row][2];
					A(row,m_numCenters+3) = 1.0;
					break;
				}
				else if(row>=m_numCenters && col<m_numCenters)
				{
