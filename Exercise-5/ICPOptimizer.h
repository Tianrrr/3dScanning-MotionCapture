...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        PoseIncrement<T> pose_increment(m_pose);

        T m_trans[3];
        T m_source[3];

        m_source[0] = T(m_sourcePoint.x());
        m_source[1] = T(m_sourcePoint.y());
        m_source[2] = T(m_sourcePoint.z());


        pose_increment.apply(m_source, m_trans);

        residuals[0] = T(m_weight) * (m_trans[0] - T(m_targetPoint.x()));
		residuals[1] = T(m_weight) * (m_trans[1] - T(m_targetPoint.y()));
		residuals[2] = T(m_weight) * (m_trans[2] - T(m_targetPoint.z()));
...
public:
    ...
    bool operator()(const T* const pose, T* residuals) const {
        ...
        m_normal[0] = T(m_targetNormal.x());
        m_normal[1] = T(m_targetNormal.y());
        m_normal[2] = T(m_targetNormal.z());
        ...
        T tmp_1 = m_normal[0]*(m_trans[0]-T(m_targetPoint.x()));
        T tmp_2 = m_normal[1]*(m_trans[1]-T(m_targetPoint.y()));
        T tmp_3 = m_normal[2]*(m_trans[2]-T(m_targetPoint.z()));


        residuals[0] = T(m_weight) * (tmp_1 + tmp_2 + tmp_3);
    ...
    void pruneCorrespondences(const std::vector<Vector3f>& sourceNormals, const std::vector<Vector3f>& targetNormals, std::vector<Match>& matches) {
        ...
        for (unsigned i = 0; i < nPoints; i++) {
            ...
            if (match.idx >= 0) {
                ...
                double cos_ang = sourceNormal.dot(targetNormal) / (sourceNormal.norm()*targetNormal.norm());
                double angle= std::acos(cos_ang) * 180 / 3.1415926;
    ...
    void prepareConstraints(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints, const std::vector<Vector3f>& targetNormals, const std::vector<Match> matches, const PoseIncrement<double>& poseIncrement, ceres::Problem& problem) const {
        ...
        for (unsigned i = 0; i < nPoints; ++i) {
            ...
            if (match.idx >= 0) {
                ...
                problem.AddResidualBlock(PointToPointConstraint::create(sourcePoint, targetPoint, 1), nullptr, poseIncrement.getData());
                ...
                if (m_bUsePointToPlaneConstraints) {
                    ...
                    // to the Ceres problem.
                    problem.AddResidualBlock(PointToPlaneConstraint::create(sourcePoint, targetPoint, targetNormal, 1), nullptr, poseIncrement.getData());
